...
class CartController extends Controller {
    ...
    public function index()
    {
        ...
        if (isset($_POST['submit'])) {
            // Lặp các phần tử trong giỏ hàng, và gán lại số lượng tương ứng cho từng phần tử
            // theo id của sản phẩm
            // Check thêm trường hợp nếu trường hợp là giá trị âm thì báo lỗi upadate
            foreach ($_SESSION['cart'] AS $product_id => $cart) {
                if ($_POST[$product_id] < 0) {
                    $_SESSION['error'] = 'Số lượng phải > 0';
                    // $url_redirect = $_SERVER['SCRIP_NAME'] . 'gio-hang-cua-ban.html';
...
//        Thêm vào giỏ -> thêm mới 1 giỏ hàng
    ...
    public function delete() {
        // Nếu trong rewrite đã có validate bằng regex, ví dụ
        // phải là số, thì bên PHP k cần validate lại nữa
        echo "<pre>";
        print_r($_GET);
        echo "</pre>";
        $product_id = $_GET['id'];
        // Xóa sản phẩm trong giỏ hàng dựa theo id bắt được
        unset($_SESSION['cart'][$product_id]);
        // Kiểm tra nếu xóa hết toàn bộ sản phẩm trong giỏ hàng rổi thì xóa luôn cái giỏ hàng
        // đó đi
