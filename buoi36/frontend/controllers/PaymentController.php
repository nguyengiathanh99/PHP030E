...
class PaymentController extends Controller {
  public function index() {
    ...
    if (isset($_POST['submit'])) {
      ...
      $note = $_POST['note'];
      $method = $_POST['method'];
      // validate form: 3 trường fullname, address và email ko dc để trống + email phải đúng
      // định dạng
      if (empty($fullname) || empty($address) || empty($mobile) || empty($email)) {
            $this->error = 'Fullname hoặc address hoặc mobile ko dc để trống';
      }
      elseif (!filter_var($email,FILTER_VALIDATE_EMAIL)) {
            $this->error = 'Email phải định dạng';
      }
      ...
      if (empty($this->error)) {
          ...
          // Xử lý cho trường payment_status: trạng thái thanh toán đơn hàng, mặc định
          // khi mới đặt hàng thì sẽ = 0
          // Bên backend sẽ cần có chức năng update lại trạng thái thanh toán
          ...
          // + Bình thường khi insert vào bảng thì sẽ trả về boolean,
          // tuy nhiên với trường hợp này sẽ cần trả về id của chính đơn hàng vừa insert
          // vì còn phải lưu vào bảng order_details
          $order_id = $order_model->insert();
          // var_dump($product_id);
          // + Sau bước trên lấy dc order_id, tiếp theo sẽ lưu tiếp vào bảng order_details - 
          // bảng này mô tả đơn hàng có các sản phẩm nào, số lượng tương ứng là bao nhiêu
          $order_detail_model = new OrderDetail();
          //  + Lặp giỏ hàng để lưu các thông tin về product_id và 
          // quantity vòa bảng order_details
          foreach ($_SESSION['cart'] AS $product_id => $cart) {
          ...
          // Gửi mail cho khách hàng, viết thành 1 phương thức gửi mail riêng
          // Nội dung mail khi gửi chính là thông tin người mua hàng

          // $body = $this->render('views/payments/mail_template order');
          // $this->sendMail($email,$body);
          // Kiểm tra phương thức thanh toán user muốn là gì, dựa vào key=method
          // + Nếu là thanh toán trực tuyến thì chuyển hướng tới trang Ngân lượng
          if ($method == 0) {
            // Tạo 1 mảng thông tin thanh toán dành cho việc hiển thị tại trang ngân lượng
